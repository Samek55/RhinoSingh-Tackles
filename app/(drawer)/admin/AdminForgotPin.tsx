...
import { requestAdminPinReset, confirmAdminPinReset } from '@/api/supabase/adminAuth';
...
export default function AdminForgotPin() {
    ...
    const handleConfirm = async () => {
        ...
        setSubmitting(true);
        try {
            const result = await confirmAdminPinReset(phoneNumber, otpCode, newPin);
            if (result.success) {
                Alert.alert('Success', 'Your PIN has been reset. Please log in.', [
                    { text: 'OK', onPress: () => router.replace('/admin/AdminLogin') },
                ]);
            } else {
                Alert.alert('Error', result.message || 'Could not reset your PIN');
            }
        } finally {
            setSubmitting(false);
        }
