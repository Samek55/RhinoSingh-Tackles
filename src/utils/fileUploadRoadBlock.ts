...
export const uploadImageToSupabaseRoadBlock = async (
  file: FileItem
) => {
  const fileExt = file.uri.split(".").pop() || "jpg";
  
  // Files will be stored directly in the "booking" folder
  const fileName = `roadblock/${Date.now()}-${Math.random()}.${fileExt}`;
...
export const uploadMultipleImagesForRoadBlock = async (
  files: FileItem[]
): Promise<string[]> => {
  try {
    const uploadPromises = files.map((file) =>
      uploadImageToSupabaseRoadBlock(file)
    );
